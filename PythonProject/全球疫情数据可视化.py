"""爬取丁香园全球疫情数据，并加入了数据库存储，和简单可视化
   数据较为宽泛，只选择了确诊，疑似，治愈，死亡四个数据
   爬取数据结果随丁香园网站而变"""
import requests
...
response = requests.get('https://ncov.dxy.cn/ncovh5/view/pneumonia')  # 获取网页响应
...
text = script.text
json_str = re.findall(r'\[.+\]', text)[0]  # 正则表达式进一步提取,匹配[]
# print(json_str)
lastDay_CoronaVirus = json.loads(json_str)  # 将json字符串转化为python对象,元素为字典的一个列表
...
while i < num:
    ...
    # 因为在这里例如tmp[i]['provinceName']返回的是字符串：美国，所以values里每一个%s都要添加双引号，并用斜杠转义，不然会抛出Unknown column '美国' in 'field list'的错误
    ...
    i = i + 1
print("更新条目:{}".format(num))  # 打印更新行数
mysql_connection.commit()
cur.close()
mysql_connection.close()

# print(dictData) # 一个以国家名称为键，国家数据字典为值的二维字典
